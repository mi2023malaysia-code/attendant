...
export async function upsertWebinar1Action(
  ...
  if (id) {
    ...
    revalidatePath('/admin/webinars1');
    revalidatePath(`/admin/webinars1/${id}`);
    redirect(`/admin/webinars1/${id}`);
  }
  ...
  revalidatePath('/admin/webinars1');
  redirect(`/admin/webinars1/${data.id}`);
}

export async function archiveWebinar1Action(formData: FormData) {
  const supabase = createPreviewSupabaseAdminClient();
  const id = readId(formData, 'id');

  if (!id) {
    redirect('/admin/webinars1');
  }
